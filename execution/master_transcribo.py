...
Chunks audio in 30-second blocks with 5-second overlapping stride.
...
def main():
    parser = argparse.ArgumentParser(description="Master Framework Verbatim Transcriber")
    parser.add_argument("--audio", required=True, help="Path to the input audio file (WAV)")
    parser.add_argument("--output", required=True, help="Path to the output JSON transcript")
    parser.add_argument("--model-dir", default="/Volumes/WORK 2TB/WORK 2026/Bronson Cancer Equity Project/Cancer Survivors/_multicam_framework/models/CrisperWhisper", help="Path to the Whisper model directory")
    ...
    try:
        ...
        print(f"[!] Failed to load model from {args.model_dir}: {e}")
        sys.exit(1)
    ...
    # Load audio
    print(f"Loading native audio... {args.audio}", flush=True)
    waveform, sr = torchaudio.load(args.audio)
    if sr != 16000:
        print(f"Resampling from {sr} to 16000...", flush=True)
    ...
    while pos < len(waveform):
        ...
        try:
            ...
            if chunk_idx % 10 == 0:  # don't spam console if there's 10 minutes of silence
                print(f"  [!] Silent/speechless chunk bypassed at {chunk_offset:.1f}s", flush=True)
            pos += chunk_size - stride_size
            chunk_idx += 1
            continue
        except Exception as e:
            print(f"  [!] Generation error at {chunk_offset:.1f}s: {e}", flush=True)
            pos += chunk_size - stride_size
            chunk_idx += 1
            continue
        
        # Decode with timestamps for word-level timing
        try:
    ...
    if filler_list[:20]: # print max 20 fillers to avoid console spam
        print("\nFiller locations (First 20):", flush=True)
        for f_str in filler_list[:20]:
